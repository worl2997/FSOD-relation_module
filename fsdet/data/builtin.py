...
def register_all_pascal_voc(root="datasets"):
    ...
    # register small meta datasets for fine-tuning stage
    # fine tuning 학습 과정에서 사용할 메타데이터 등록
    for prefix in ["all", "novel"]:
            ...
            for shot in [1, 2, 3, 5, 10]:
                for year in [2007, 2012]:
                    for seed in range(30):
                        ...
                        # all_1shot_split_1_trainval ? 이런게 있나
    ...
    for name, dirname, split, keepclasses, sid in METASPLITS:
        ...
        register_meta_pascal_voc(
            ...
            _get_builtin_metadata("pascal_voc_fewshot"), # builtin_meta.py 참조 , ret 파일 (메타데이터 로드)
            os.path.join(root, dirname), # root : datasets , dirname: VOC2007 / VOC2012
            split, # {}_{}shot_split_{}_trainval  ex) all_1shot_split_1_trainval, novel_1shot_split_1_trainval
            year,
            keepclasses, # ex) base_novel_1, novel1
            sid, # split id -> 1,2,3
        )

        # step1 -> register meta information
        MetadataCatalog.get(name).evaluator_type = "pascal_voc" # set evaluator
