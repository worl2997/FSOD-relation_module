...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        '''
        batched_inputs ->
        file_name, image_id, height.width, voc_glove, image, instances

        {'file_name': 'datasets/VOC2007/JPEGImages/009923.jpg', 'image_id', 'height', 'width','voc_glove' , 'annotations'
        ...
        word2vec = batched_inputs[0]['voc_glove']

        # 수정)roi_head 부분에 word2vec 추가하였음
        _, detector_losses = self.roi_heads(
            images, features, proposals, word2vec, gt_instances
        )
    ...
    def inference(
        ...
        if detected_instances is None:
            ...
            results, _ = self.roi_heads(images, features, proposals, word2vec, None)
        else:
            detected_instances = [
