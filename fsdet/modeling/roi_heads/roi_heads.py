...
class StandardROIHeads(ROIHeads):
    ...
    # 수정 word2vec 추가
    def forward(self, images, features, proposals, voc_word2vec, targets=None):
        """
        See :class:`ROIHeads.forward`.
        """
        del images
        ...
        if self.training:
            losses = self._forward_box(features_list, proposals,voc_word2vec)
            return proposals, losses
        else:
            pred_instances = self._forward_box(features_list, proposals,voc_word2vec)
            return pred_instances, {}
    ...
    #
    # 이 부분 수정
    def _forward_box(self, features, proposals, word2vector):
        """
        ...
        # box_features -> 2개의 feature가 포함된 리스트 [box_feat, cls_feat]
        # 1024개 sample들 1024, 300 차원의 벡터들
        # 추출된 박스 피쳐를 기반으로 predictor 수행
        # Predictor는 modeling-> fast_rcnn.py -> FasterRCNNOutputLayers 참고

        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features, word2vector) # word2vec 데이터 삽입, relation 모듈포함
        del box_features
                # loss 계산을 위한 인스턴스
        outputs = FastRCNNOutputs(
            self.box2box_transform,
