...
def ckpt_surgery(args):
    ...
    Novel 데이터 세트에 대한 미세 조정을 위해 최종 레이어 가중치를 제거하거나
    Novel 클래스에 대해 무작위로 초기화된 가중치를 추가합니다.

     참고: LVIS용 기본 감지기는 모든 클래스에 대한 가중치를 포함하지만
     기본 클래스에 해당하는 가중치만 기본 교육 중에 업데이트됩니다(이 디자인 선택에는 특별한 이유가 없습니다).
     따라서 임의 초기화 단계는 실제로 필요하지 않습니다.
    """
