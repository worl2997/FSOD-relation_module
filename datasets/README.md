...
The shots in the `vocsplit` directory are the same shots used by previous works. We additionally sample 29 more groups of shots for a total of 30 groups, which can be generated by using [prepare_voc_few_shot.py](../tools/prepare_voc_few_shot.py).

See [prepare_voc_few_shot.py](../tools/prepare_voc_few_shot.py) for generating the seeds yourself.
